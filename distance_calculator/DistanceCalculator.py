...
class DistanceCalculator:
    ...
    def create_detection_image(self, disparity_map, detected_objects):
     ...
     def detect_closest_object(self, disparity_map, disparity_threshold, min_region_area=500):
        """Detect the closest object based on disparity thresholding.

        Args:
            disparity_map (np.array): Disparity map from stereo vision
            disparity_threshold (float): Disparity value corresponding to the distance threshold
            min_region_area (int): Minimum area of the region to be considered valid

        Returns:
            tuple: (closest_distance, (x, y), region_area) or (inf, None, 0)
        """
        # Create a binary mask where disparity is greater than the threshold
        close_objects_mask = cv2.threshold(disparity_map, disparity_threshold, 255, cv2.THRESH_BINARY)[1].astype(np.uint8)

        # Apply morphological operations to reduce noise
        kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (5, 5))
        close_objects_mask = cv2.morphologyEx(close_objects_mask, cv2.MORPH_OPEN, kernel)

        # Find contours in the binary mask
        contours, _ = cv2.findContours(close_objects_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
