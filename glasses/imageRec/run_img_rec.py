...
def run_image_recognition(input_path, confidence_threshold=0.5, save_result=False, show_result=True):
    ...
    Args:
        ...
        show_result (bool, optional): Whether to display the images with predictions. Defaults to True.
    """
    img_rec = ImgRec()
    ...
    if os.path.isdir(input_path):
        ...
        results = img_rec.predict_folder(input_path, confidence_threshold=confidence_threshold, save_result=save_result, show_result=show_result)
        ...
        detections = img_rec.predict_one_image(input_path, confidence_threshold=confidence_threshold, save_result=save_result, show_result=show_result)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python run_image_recognition.py <input_path> [confidence_threshold] [save_result] [show_result]")
    ...
    show_result = sys.argv[4].lower() == 'true' if len(sys.argv) > 4 else True

    run_image_recognition(input_path, confidence_threshold, save_result, show_result)
